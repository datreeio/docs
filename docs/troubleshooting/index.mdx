...
Some of k8s version releases consist of deprecated APIs.  
When validating objects with a deprecated API version, the CLI will not be able to find the schema since it's removed for the version that you validate against.  
For example, the beta API versions of Ingress are no longer served from K8s version 1.22.
<Tabs>
