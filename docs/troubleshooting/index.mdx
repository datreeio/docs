...
``` 
It probably means that you don't have a Kubernetes cluster up and running. To be able to install the webhook, make sure you have kubectl installed and an accessible Kubernetes cluster, only then try to install Datree again.

#### Windows users

If you are a Windows user, you can not install Datree webhook within PowerShell.
Instead, you should use WSL for running linux based machine on Windows.
