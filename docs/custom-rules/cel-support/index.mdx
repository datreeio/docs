...
- The `schema` property **must** have a property named `CELDefinition`.
- The `CELDefinition` property **must** have a property named `expression` that contains the logic of the rule written in CEL.
- The `CELDefinition` property **may** have a property named `message`, which specifies what print when the expression is violated.
