...
1. Create a `datree` namespace which will hold Datree's services and application resources:
```
kubectl create namespace datree
```

2. Install Datree using one of the following methods:  

**Option 1 - Using the Argo CLI**  
Replace `<APP_NAME>` and `<DATREE_TOKEN>` with your desired values, then run the following command:

```    
argocd app create <APP_NAME> --repo https://github.com/datreeio/admission-webhook-datree.git --path charts/datree-admission-webhook --dest-namespace datree --dest-server https://kubernetes.default.svc --helm-set datree.token=<DATREE_TOKEN> --helm-set datree.clusterName=$(kubectl config current-context)
```

**Option 2 - Using the Argo UI**  
Create a new application, then edit the YAML and copy-paste the following (replace `<APP_NAME>`, `<DATREE_TOKEN>` and `<CLUSTER_NAME>` with your desired values):
...
spec:
    ...
    source:
        path: charts/datree-admission-webhook
        repoURL: 'https://github.com/datreeio/admission-webhook-datree.git'
        targetRevision: HEAD
        helm:
            parameters:
                - name: datree.token
