...
There is no need to set up anything because every Kubernetes manifest that Datree scanned is also validated before the rules from the [Centralized policy](/dashboard/centralized-policy) are executed. &#x20;
...
![schema_default_version](/img/cli/schema-validation/3.png)
