---
title: ☑️ Prevent exposed secrets in BitBucket objects
slug: /built-in-rules/prevent-exposed-secrets-bitbucket
---

import CommonOverview from '../common-overview.mdx'
import CommonHowToFix from '../common-how-to-fix.mdx'

<CommonOverview />

**Policy as code identifier:** ALL_EXPOSED_SECRET_BITBUCKET


---

## This rule will fail

If a BitBucket **client ID** and/or **client secret** are present anywhere in the manifest:

```yaml
CLIENT_ID: bitbucketr=>yf8gf86gcdz1e4df2efnbtmcpzwivk1a
SECRET: bitbucketnv-sl8p.rdr70z=>='`-y53xqehjw62a1z3wmh9-np9zkbl54_pf9isl0rvinzy3w2qwls0im2u=i08_6-5
```

### Rule output in the CLI

```terminal
$ datree test *.yaml

>> File: failExample.yaml
❌ Prevent exposed secrets in BitBucket objects [1 occurrence]
💡 Secret data found in manifest - keep your sensitive data elsewhere to prevent it from being stolen 
```

---

## How to fix this failure

<CommonHowToFix />

---

## Read more

- [Distribute Credentials Securely Using Secrets](https://kubernetes.io/docs/tasks/inject-data-application/distribute-credentials-secure/)
