...
If rollout pods get stuck in an error state (e.g. image pull back off), the rollout degrades after the progress deadline is exceeded but the bad ReplicaSet/Pods aren't scaled down. The pods would keep retrying and eventually the rollout message would read _ProgressDeadlineExceeded: The replicaset \<\name> has timed out progressing._
...
- [**_Github memory_**](https://githubmemory.com/repo/argoproj/argo-rollouts/issues/1593)
