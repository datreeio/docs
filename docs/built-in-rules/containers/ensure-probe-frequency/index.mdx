---
title: ☑️ Ensure container probes have a configured frequency
...
The field `periodSeconds` defines how often (in seconds) to perform the probe. It has a default value of 10 seconds, and a minimum value of 1 second.
...
❌ Ensure container probes have a configured frequency [1 occurrence]
💡 Incorrect value for key `periodSeconds` - set to prevent unwanted default frequency behavior
```
...
Each each container probe has a configured `periodSeconds` property with a minimum value of 1
