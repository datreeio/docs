...
It is possible to run Datree as a standalone in an air-gapped environment by downloading [Datree’s binary](https://github.com/datreeio/datree/releases/latest) manually and copying it to your offline environment and adding `offline=local` to your local config file:  
```terminal
...
```terminal
E.g: datree test [manifest] --policy-config [policyAsCodeFile]
```
```
