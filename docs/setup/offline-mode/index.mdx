...
3. Set `offline=local` in your local [config file](/setup/cli-arguments/#configure-the-cli):  
```terminal
...
```terminal
datree test [manifest] --schema-location '[schemaVersionDir]/{{ .ResourceKind }}{{ .ResourceAPIVersion }}.json'
```
```terminal
E.g: datree test k8s-demo.yaml --schema-location 'v1.21.0-standalone-strict/{{ .ResourceKind }}{{ .ResourceAPIVersion }}.json'
```

:::info
Note that '{{ .ResourceKind }}{{ .ResourceAPIVersion }}.json' does not need to be replaced with an actual filename, the CLI will be able to access all .json files with this name format in the specified directory.
:::
