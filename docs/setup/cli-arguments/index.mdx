...
1. `token` - the [token](/setup/account-token) used to link the CLI to your dashboard.
...
The default behavior (`fail`) will cause the CLI to exit with an error.  
When in [offline-mode](/setup/offline-mode), you will need to instruct the CLI to use a local policy file instead. Set the value to `local` by running the following command:  
```terminal
