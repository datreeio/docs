...
| --schema-location        | N/A   | e.g. "http\://example.com/schema.json" | Override Kubernetes schema location. See [relevant page](https://hub.datree.io/schema-validation#m2-overriding-schemas-location-crd-support) for more information. |
...
For more information, options and examples, see the [Kustomize support](docId:drNWV3HERFRm43eXIv7NA) page.

### Publish policy-as-code configuration

When using [Policy as code](docId:b98paugYVzlfox0ztnfem) mode, the command `datree publish fileName.yaml` will publish policies configuration for given fileName. Input should be the path to the Policy-as-Code yaml configuration file.
