...
Every account has its own token. The token connects the policy checks to your [Centralized policy](/setup/centralized-policy) to know which policies and rules to run. When a policy check is triggered, Datree attempts to read the account token from the following locations (in order):
