...
There is no need to set up anything because every Kubernetes manifest that Datree scanned is also validated before the rules from the [Centralized policy](docId:0-uWz2hrYE6OvSK15gWNK) are executed. &#x20;
...
<img src='/img/setup/schema-validation/3.png' alt='' />
...
When the --schema-location parameter is not used, or set to "default", the CLI will be downloading schemas from [kubernetes-json-schema](https://github.com/yannh/kubernetes-json-schema). The CLI supports passing one, or multiple, schemas locations - HTTP(s) URLs, or local filesystem paths - in which case it will look for schema definitions in each of them, in order, stopping as soon as a matching file is found.
