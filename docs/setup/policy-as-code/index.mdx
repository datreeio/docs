...
Once the Policy-as-code (PaC) mode is enabled, the only way to change the policies in your account is by publishing a YAML configuration file ([policies.yaml](https://hub.datree.io/policy-as-code#go-policiesyaml)) with the defined policies.
...
To change the policies in your account you will need to update the policies configuration YAML file ([policies.yaml](https://hub.datree.io/policy-as-code#go-policiesyaml)) and publish it:
