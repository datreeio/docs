...
"Centralized policy" is the concept of controlling distributed policy execution from a centralized location. This concept enables the policy owner to easily control the rules that are evaluated in each run of Datree without creating operation overhead. Logging onto the dashboard allows you to manage the centralized policy.

## Enable and disable rules in a policy

Each of the [53 built-in](/built-in-rules) rules can be toggled "ON" or "OFF" in the [dashboard](https://app.datree.io) to suit your needs. Once a rule is disabled or enabled, it will automatically propagate to all policy checks executed against that policy (via [account token](/setup/account-token)). This way, the policy owner is not required to update all devices linked with this policy individually.

![centralized_policy_dashboard](/img/setup/centralized-policy/1.png)

## Edit rule fail suggestion text

When a rule fails, the output text displayed in the CLI is editable. The policy owner can use this to educate policy consumers (e.g., developers, teammates, etc.) and inform them how to fix their misconfiguration.
...
Build a new policy and name it something descriptive.
