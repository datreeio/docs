...
In the [dashboard](https://app.datree.io), each of the [44 built-in](/built-in-rules) rules can be switched "ON" or "OFF" according to your needs. Once a rule is disabled or enabled, it will automatically propagate to all policy checks that are executed against that policy (via [account token](/setup/account-token)). This way, the policy owner doesn't need to manually update all the devices connected to this policy.
