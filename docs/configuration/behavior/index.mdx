...
run `kubectl rollout restart deployment -n datree` to restart the webhook.
and `kubectl get job "scan-job" -n datree -o json | jq 'del(.spec.selector)' | jq 'del(.spec.template.metadata.labels)' | kubectl replace --force -f -` to rescan your cluster and apply your changes.
:::
