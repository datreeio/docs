...
Datree can be configured to skip certain rules by using a dedicated `ConfigMap` (see examples below).  
...
* Enter the resource/s you wish to skip under the `datree.customSkipList` key, in the format of *namespace;resourceKind;resourceName*.
* You can use a regular expression for any of the aformentioned properties.

**For example**, the following command will cause Datree to skip all resources of kind `ServiceAccount` in all namespaces, regardless of their name:
```terminal
helm upgrade -n datree datree-webhook ./charts/datree-admission-webhook --reuse-values --set datree.customSkipList[0]="(.*);ServiceAccount;(.*)" --debug
```

:::info Important
After updating the `ConfigMap`, run `kubectl rollout restart deployment -n datree` to restart the webhook and apply your changes.
:::
...
The following example will cause Datree to skip all resources in all namespaces if the resource name starts with the prefix 'appName-':
```terminal
"(.*);(.*);(^appName-.*)"
```
