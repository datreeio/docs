...
```terminal
helm repo update
helm upgrade -n datree datree-webhook datree-webhook/datree-admission-webhook --reuse-values --set image.tag=null --set scanJob.image.tag=null --debug
```

:::tip FYI
Setting the `image.tag` parameters to `null` instructs Datree to automatically use the latest image where needed.
:::
