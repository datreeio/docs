...
Then, we recommend setting guardrails on your clusters using Datree’s [admission webhook](https://github.com/datreeio/datree/tree/main/cluster-integration), using the policy you just customized. The webhook will catch commands that create or change resources in your cluster, and run policy checks against them. This will establish your current status as a standard for your production environment that will now be automatically kept.
...
Make sure you standardize how your developers should fix violated rules by taking advantage of the customizable [error message](/dashboard/centralized-policy#edit-rule-fail-suggestion-text) that is displayed when a rule fails.
