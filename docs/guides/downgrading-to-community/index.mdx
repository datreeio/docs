...
Datree core components are open-source and therefore, you can always use them (free forever) in your own environment.
...
You can use the `policies.yaml` file with Datree CLI by passing it as an argument to the `--policy-config` flag:
...
Yes, you can. The process will be the same but instead of publishing your `policies.yaml` file to your dashboard, you can use it locally with the `--policy-config`.

### What will happen to my data when I downgrade?
Nothing. Your data will still be sent to your dashboard so once you will upgrade back, your data will still be up-to-date.
...
Yes, you can. You will not be blocked in any way from using Datree CLI in your CI.
